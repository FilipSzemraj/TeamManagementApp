...
import {Composer, GiftedChat, InputToolbar, MessageImage} from 'react-native-gifted-chat';
...
const SpecIndividual = ({ navigation, route }) => {
    ...
    {/*useFocusEffect(
        useCallback(() => {
            const sendLocationMessage = async () => {
                const { selectedLocation } = useContext(UserContext); // Pobranie lokalizacji z kontekstu
        ...
        }, [useContext(UserContext).selectedLocation]) // Dodaj selectedLocation do listy zależności
    );*/}
    ...
    const onSend = useCallback(async (messages = []) => {
        ...
        if (photoUri) {
            ...
            try {
                ...
                // Obsłuż błąd, możliwe że chcesz kontynuować bez dodawania obrazu
            }
            finally{
                await AsyncStorage.removeItem('lastPhoto');
            }
    ...
    const renderMessage = (props) => {
        if (props.currentMessage && props.currentMessage.location) {
            // Niemożliwe jest zastosowanie domyślnego renderowania z GiftedChat dla niestandardowych warunków,
            // dlatego dla wiadomości zawierających lokalizację renderujemy ikonę
            return (
                <TouchableOpacity
                    onPress={() => {
        ...
        // Dla innych wiadomości nie stosujemy żadnych zmian, pozwalając GiftedChat na używanie domyślnego renderowania
